...
void SetAmmoCookie(int client, bool enabled) {
  ...
  SetClientCookie(client, cookieInfiniteAmmo, value);
}

bool GetAmmoCookie(int client) {
  char value[2];
  GetClientCookie(client, cookieInfiniteAmmo, value, sizeof(value));
  if (strlen(value) == 0) return false;
  g_bInfiniteAmmo[client] = (StringToInt(value) != 0);
  return true;
}
...
void SetImmunityCookie(int client, bool enabled) {
  ...
  char value[2];
  IntToString(enabled ? 1 : 0, value, sizeof(value));
  SetClientCookie(client, cookieImmunity, value);
}

bool GetImmunityCookie(int client) {
  char value[2];
  GetClientCookie(client, cookieImmunity, value, sizeof(value));
  if (strlen(value) == 0) return false;
  g_bImmunity[client] = (StringToInt(value) != 0);
  return true;
}
